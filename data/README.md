...
### Experiment 2
The directed networks with branch lengths used for the second simulation experiments in the Squirrel paper were generated with part of the script `simulate_alignments.py`. The generated networks are obtained from the semi-directed network from the first experiment and are in the zip-file `generated_dnetworks.zip` (using the eNewick format). The filenames for the networks have the format `WDNXXXXX_LYYYYY_RZZZZZ.txt`, where `XXXXX` is the id-number the corresponding semi-directed network, `YYYYY` the number of leaves, and `ZZZZZ` the number of reticulations. The sequence alignments that were simulated for the second experiment are available .... (in `.nexus` format) and were also generated with the script `simulate_alignments.py`. The sequence alignments are in the format `SEQXXXXX_LYYYYY_RZZZZZ_LENAAAAAAAAAAAA.nexus`, where `XXXXX` is the id-number the corresponding semi-directed network, `YYYYY` the number of leaves, `ZZZZZ` the number of reticulations, and `AAAAAAAAAAAA` the length (in bp) of the sequences. The numerical results of the experiment are in the file `results_experiment2.csv` and are computed with the script `experiment2.py`.
